...
//               MaterialPageRoute(
//                 builder: (_) => DashboardScreen(
//                   patientMrNo: '', // Empty or default MR number
...
class CustomMessageDialog {
  ...
  // Show success message dialog with tick icon
  static void showSuccess(BuildContext context, String message, {VoidCallback? onSuccess}) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext context) {
...
class _ForgotPasswordDialogState extends State<ForgotPasswordDialog> {
  ...
  Future<void> _sendOtp() async {
    ...
    try {
      ...
      if (response['success'] == true) {
        _startTimer();
        CustomMessageDialog.showSuccess(context, response['message'] ?? 'OTP sent successfully');
      } else {
        CustomMessageDialog.showError(context, 'Failed to send OTP');
      }
    } catch (e) {
      CustomMessageDialog.showError(context, 'Error sending OTP: ${e.toString()}');
    } finally {
      setState(() => _isSendingOtp = false);
    }
  }
  ...
  Future<void> _verifyOtp() async {
    ...
    if (otp.isEmpty) {
      CustomMessageDialog.showError(context, 'Please enter OTP');
      return;
    }
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        setState(() => _isVerifyingOtp = false);
        CustomMessageDialog.showError(context, response['message'] ?? "Invalid OTP");
      }
  ...
  Future<void> _updatePassword() async {
    ...
    try {
      ...
      CustomMessageDialog.showError(context, 'Failed to update password: ${e.toString()}');
    } finally {
      setState(() => _isLoading = false);
    }
  }
...
class _LoginScreenState extends State<LoginScreen> with WidgetsBindingObserver {
  ...
  Future<void> _login() async {
    ...
    if (contactNo.isEmpty || password.isEmpty) {
      CustomMessageDialog.showError(context, 'Please enter both contact number and password');
      return;
    }

    if (contactNo.length < 10) {
      CustomMessageDialog.showError(context, 'Please enter a valid contact number');
      return;
    }
    ...
    try {
      print('Attempting login for: $contactNo');
      ...
      if (mrNo.isEmpty) {
        CustomMessageDialog.showError(context, 'MR Number not found');
        return;
      }
      ...
      print('Login error: $e');
      if (mounted) {
        CustomMessageDialog.showError(context, 'Invalid username or password');
      }
    } finally {
      if (mounted) {
        setState(() => _loading = false);
